<template>
  <div class='cart'>
      ...
      <div class="cart-content">
        <div class="cart-content-main" v-for="(item,index) in cartList">
          ...
          <div class="cart-delete">
            <span class="cart-control-delete" @click="handleDelete(index)">刪除</span>
          </div>
        </div>
      ...
      <div class="cart-promotion" v-show="cartList.length">
        <span>使用优惠券</span>
        <input type="text" v-model="promotionCode" style="padding: 2px; margin:0 2px;">
      ...
      <div class="cart-footer" v-show="cartList.length">
        ...
        <div>
          ...
          <template v-if="promotion" >
            (优惠<span style="color: red;"> {{promotion}}</span>)
          </template>
        </div>
        <div class="cart-footer-desc">
          <div @click="handleOrder" class="cart-control-order">现在结算</div>
        </div>
      </div>
  </div>
</template>
...
export default {
  computed: {
    ...
    costAll () {
      ...
      // costall = costall + 1
      this.cartList.forEach(item => {
        costall += this.productDictList[item.id].cost * item.count
      })
      // console.log(costall, 'dsad')
      return costall
    }
  },
  data () {
  ...
  methods: {
    ...
    handleCheckCode () {
      console.log(this.promotionCode, 'Vue.js')
...
.cart{
  margin: 32px;
  background: #fff;
  border : lpx solid #dddee1 ;
  border-radius: l0px;
}
.cart-header-title{
padding : 16px 32px ;
border-bottom: lpx solid #dddee1;
border-radius: l0px l0px 0 0;
background: #f8f8f9;
}
.cart-header-main{
padding: 8px 32px;
...
.cart-content-main{
  ...
  line-height : 60px;
  text-align : center;
  border-bottom: lpx dashed #e9eaec ;
  overflow : hidden ;
}
